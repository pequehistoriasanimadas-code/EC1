...
(function installOutput0326(){
  ...
  const baseApply=applyDesign;applyDesign=function(next={}){const r=baseApply(next);syncCustomFonts(design?.customFonts||next?.customFonts||[]).catch(()=>{});return r;};
  ...
  async function prepareVoice(url,serial){try{audio.pause();audio.currentTime=0;}catch{}if(!url){try{audio.removeAttribute('src');audio.load();}catch{}return true;}audio.preload='auto';audio.dataset.ecSerial=String(serial);audio.src=url;try{audio.load();}catch{}return waitAudioReady(serial);}
