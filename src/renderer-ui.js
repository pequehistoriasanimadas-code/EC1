...
function humanFeedStatus(r){
  ...
  if(r.ok){const fallback=/FALLBACK/i.test(r.mode||'');return{label:`${fallback?'Funcionando mediante fuente alternativa':'Funcionando'} · ${r.count||0} noticias`,cls:'ok',dot:'ok'};}
  return{label:'No pudimos leer esta fuente',cls:'error',dot:'error'};
}
async function testFeedAt(i,silent=false){
  const f=settings.rssFeeds[i];if(!f)return null;if(!f.enabled){feedHealth[f.id]={paused:true};renderFeeds();return null;}
  feedHealth[f.id]={checking:true};renderFeeds();
  try{const r=await window.ECAPI.testRss(f);feedHealth[f.id]=r;if(!silent)status(r.ok?`${f.name}: ${r.count} noticias disponibles`:`No pudimos leer ${f.name}`);renderFeeds();return r;}
  catch(e){feedHealth[f.id]={ok:false,error:e.message||String(e),mode:'ERROR'};if(!silent)status(`No pudimos comprobar ${f.name}`);renderFeeds();return null;}
}
...
async function refreshRuntimeStatus(){
  try{
    const s=await window.ECAPI.localStatus(),ready=s.runtime&&s.model;
    $('#localInfo').textContent=!s.model?'IA local no descargada. Puedes descargarla para usar EC sin depender de una API.':s.running?'IA local preparada y activa ✓':'IA local lista ✓ · se activará automáticamente cuando sea necesaria.';
    $('#downloadModel').disabled=!!s.model||!!s.downloading;$('#startLocal').disabled=!s.model||s.running;$('#stopLocal').disabled=!s.running;
  }catch(e){$('#localInfo').textContent='No se pudo comprobar la IA local.';$('#startLocal').disabled=true;$('#stopLocal').disabled=true;}
  try{
    const t=await window.ECAPI.ttsStatus();
    $('#ttsInfo').textContent=t.ready?(t.workerRunning?'Motor de voz preparado ✓ · permanece listo entre notas':'Motor de voz disponible ✓ · se preparará al generar la primera locución'):'El motor de voz no está disponible en esta instalación.';
    const v=$('#voice'),previous=settings?.tts?.voice||v.value;v.innerHTML='';const voices=(t.voices||[]).filter(x=>/^e[fm]_/.test(x)),usable=voices.length?voices:(t.voices||[]).slice(0,80);
    for(const name of usable){const o=document.createElement('option');o.value=name;o.textContent=voiceLabel(name);v.appendChild(o);}if(!v.options.length){const o=document.createElement('option');o.value='';o.textContent='No se encontraron voces';v.appendChild(o);v.disabled=true;}else{v.disabled=false;if([...v.options].some(o=>o.value===previous))v.value=previous;else{v.value=v.options[0].value;if(settings?.tts)settings.tts.voice=v.value;}}
  }catch(e){$('#ttsInfo').textContent='No se pudo cargar la lista de voces.';$('#voice').innerHTML='<option value="">Voces no disponibles</option>';$('#voice').disabled=true;}
}
async function refreshPronunciationStatus(){
  try{const p=await window.ECAPI.pronunciationStatus(),learned=Number(p.learningEntries??p.cacheEntries??0);$('#pronunciationInfo').textContent=`Pronunciación automática activa ✓ · ${learned} término${learned===1?'':'s'} aprendido${learned===1?'':'s'}${p.model?' · mejora local disponible':''}`;$('#downloadPronunciationModel').disabled=!!p.model;$('#pronunciationLearningCount').textContent=`${learned} aprendidas`;}
  catch{$('#pronunciationInfo').textContent='Pronunciación básica disponible.';}
}
...
async function refreshDocuments(){
  ...
  try{documentScan=await window.ECAPI.documentList();if(documentScan.folder){settings.documents.folder=documentScan.folder;$('#documentFolderInfo').textContent=documentScan.folder;}$('#documentCount').textContent=`${documentScan.count||0} documento${documentScan.count===1?'':'s'}`;renderDocuments();}
  catch(e){box.innerHTML='<div class="empty">No se pudo leer la carpeta de documentos.</div>';}
}
function renderDocuments(){
  const box=$('#documentList');if(!box)return;const files=documentScan.files||[];
  if(!files.length){box.innerHTML='<div class="empty">No se encontraron archivos TXT o DOCX.</div>';return;}
  box.innerHTML=files.map((x,i)=>{
    const image=x.imageUrl?`<div class="document-image" style="background-image:url('${escapeHtml(x.imageUrl.replace(/'/g,'%27'))}')"></div>`:'<div class="document-image"></div>';
    const cat=x.categoryFromFolder||'Categoría automática',img=x.imageSource==='document'?'imagen propia':x.imageSource==='category'?'imagen de categoría':'imagen de respaldo';
    return`<div class="document-item ${x.processed?'processed':''}"><input class="document-check" type="checkbox" data-doc-i="${i}" ${x.processed?'disabled':''}>${image}<div class="document-copy"><div class="document-title">${escapeHtml(x.title||x.name)}</div><div class="document-meta">${escapeHtml(cat)} · ${escapeHtml(x.ext.toUpperCase())} · ${escapeHtml(img)}</div></div><span class="document-badge ${x.processed?'done':''}">${x.processed?'Procesada':'Pendiente'}</span></div>`;
  }).join('');
}

async function loadNews(){
  status('Actualizando noticias…');
  try{const r=await window.ECAPI.loadRss();stories=r.items||[];for(const f of r.feedStatus||[])feedHealth[f.id]=f;renderFeeds();renderNews();status(`${stories.length} noticias disponibles${r.errors?.length?` · ${r.errors.length} fuente${r.errors.length===1?'':'s'} con problemas`:''}`);}catch(e){status('No se pudieron actualizar las noticias. Revisa tus fuentes.');}
}
...
async function saveSettings(options={}){
  if(!settings)throw new Error('Los ajustes todavía no están disponibles');normalizeProviders(true);updateDesignFromControls(false);
  ...
  settings.documents={...settings.documents,targetSeconds:Number($('#documentDuration').value)||60,categoryMode:$('#documentCategory').value||'auto',batchDate:$('#documentBatchDate').value||'',priority:$('#documentPriority').value||'normal',watch:$('#documentWatch').checked};
  settings.automation.bufferReady=Math.max(1,Math.min(30,Number($('#bufferReady').value)||15));settings.automation.queueMax=Math.max(settings.automation.bufferReady,30);settings.automation.maxAgeHours=Math.max(1,Math.min(48,Number($('#maxAge').value)||6));settings.automation.avoidRepeats=$('#avoidRepeats').checked;settings.visual.pauseSeconds=Math.max(0,Math.min(10,Number($('#pauseSeconds').value)||2.5));settings.visual.queueColors=readQueueColors();
  const r=await window.ECAPI.saveSettings(settings);settings.ai.hasClaudeKey=!!r.hasClaudeKey;settings.ai.hasGeminiKey=!!r.hasGeminiKey;settings.ai.claudeModel=r.claudeModel||'claude-haiku-4-5-20251001';settings.ai.claudeKey='';settings.ai.geminiKey='';$('#claudeKey').value='';$('#geminiKey').value='';refreshProviderUi();$('#ttsPerformanceHint').textContent=ttsProfileHint(settings.tts.resourceMode);if(!options.quiet){await refreshRuntimeStatus();await refreshPronunciationStatus();status('Todos los cambios fueron guardados');}return r;
}
