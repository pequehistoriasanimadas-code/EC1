...
(function installRenderer0332(){
  ...
  let latest=null,raf=0,rendering=false,lastAutomationStateAt=0,sessionPollTimer=null;
  ...
  function metaText(row){
    ...
    const stall=seconds(Number(m.ttsStallMs)>0?Number(m.ttsStallMs)/1000:NaN);if(stall)timings.push(`Bloqueo voz ${stall}`);
    ...
    const rtf=Number(m.ttsRealtimeFactor||0);if(rtf>0)timings.push(`RTF ${rtf.toFixed(2)}`);
  ...
  function technicalHtml(row){const m=row?.metrics||{},bits=[];if(row?.provider)bits.push(`Proveedor: ${esc(row.provider)}`);if(row?.model)bits.push(`Modelo: ${esc(row.model)}`);if(row?.stage)bits.push(`Etapa: ${esc(stageLabel(row.stage))}`);if(m.pipelineMode)bits.push(`Pipeline: ${esc(pipelineLabel(m.pipelineMode))}`);if(m.ttsEngine){const variant=m.ttsEngine==='chatterbox'?(m.ttsVariant==='latam'?' · LatAm':m.ttsVariant==='multilingual'?' · Multilingual V3':''):'';bits.push(`Voz: ${esc(m.ttsEngine)}${variant}${m.ttsDevice?` · ${esc(String(m.ttsDevice).toUpperCase())}`:''}`);}if(m.ttsGpuName)bits.push(`GPU voz: ${esc(m.ttsGpuName)}${Number(m.ttsGpuVramMb)>0?` · ${(Number(m.ttsGpuVramMb)/1024).toFixed(1)} GB`:''}`);if(Number(m.ttsRealtimeFactor)>0)bits.push(`RTF: ${Number(m.ttsRealtimeFactor).toFixed(2)}`);if(Number(m.ttsSpeed)>0&&Math.abs(Number(m.ttsSpeed)-1)>.001)bits.push(`Velocidad: ${Number(m.ttsSpeed).toFixed(2)}x`);if(Number(m.ttsRetryCount)>0)bits.push(`Reintentos voz: ${Number(m.ttsRetryCount)}`);if(Number(m.ttsStallMs)>0)bits.push(`Bloqueo detectado: ${seconds(Number(m.ttsStallMs)/1000)}`);if(Number(m.ttsProgressCount)>0)bits.push(`Progreso TTS: ${Number(m.ttsProgressCount)} eventos`);if(m.gpuQueueWatchdog)bits.push(`Watchdog GPU: espera excedió ${seconds(Number(m.gpuQueueTimeoutMs||0)/1000)}`);if(m.ttsLastError)bits.push(`Último error voz: ${esc(String(m.ttsLastError).slice(0,180))}`);if(row?.isExclusive&&row?.exclusiveContentMode)bits.push(`Contenido exclusivo: ${row.exclusiveContentMode==='full'?'completo':row.exclusiveContentMode==='public-preview'?'vista pública':esc(row.exclusiveContentMode)}`);if(row?.storyUrl)bits.push(`URL: ${esc(row.storyUrl)}`);if(!bits.length)return'';return`<details class="technical-details"><summary>Ver detalles técnicos</summary><div>${bits.join(' · ')}</div></details>`;}
  ...
  function signature(row,index){
    const m=row?.metrics||{};
    return JSON.stringify([row?.displayPosition,index,row?.title,row?.status,row?.sourceType,!!row?.isExclusive,!!row?.exclusiveBlocked,row?.planText,row?.feedName,row?.category,row?.error,row?.stage,row?.provider,row?.model,row?.exclusiveContentMode,m.aiQueueWaitMs,m.textElapsedMs,m.textTokensPerSec,m.pronunciationElapsedMs,m.ttsQueueWaitMs,m.ttsElapsedMs,m.audioDurationSec,m.totalElapsedMs,m.ttsRealtimeFactor,m.ttsEngine,m.ttsDevice,m.ttsVariant,m.ttsGpuName,m.ttsGpuVramMb,m.ttsRetryCount,m.ttsStallMs,m.ttsProgressCount,m.ttsLastError,m.pipelineMode,typeColor(row)]);
  }
  ...
  function syncSessionCounters(snapshot){
    const x=snapshot?.session||{},news=q('#sessionNewsEmitted'),canned=q('#sessionCannedEmitted'),ads=q('#sessionAdsEmitted');
    if(news)news.textContent=String(Number(x.newsEmitted)||0);
    if(canned)canned.textContent=String(Number(x.cannedEmitted)||0);
  ...
  if(refreshBase)refreshAutomation=function(s){const r=refreshBase(s);acceptAutomationState(s);return r;};

  window.ECAPI.on('automation:state',s=>acceptAutomationState(s));
  window.ECAPI.automationStatus?.().then(s=>acceptAutomationState(s)).catch(()=>{});
  sessionPollTimer=setInterval(()=>{if(document.hidden||Date.now()-lastAutomationStateAt<2200)return;window.ECAPI.automationStatus?.().then(s=>acceptAutomationState(s)).catch(()=>{});},2500);
  window.addEventListener('beforeunload',()=>clearInterval(sessionPollTimer),{once:true});
})();
