...
(function installRenderer0332(){
  ...
  function stageLabel(stage=''){return({article:'artículo','ai-wait':'esperando Qwen / IA',ai:'generando con Qwen / IA',pronunciation:'pronunciación','tts-wait':'esperando voz / GPU',tts:'generando voz','tts-retry-wait':'reintentando voz · GPU liberada',ready:'lista'}[String(stage)]||String(stage||''));}
  ...
  function signature(row,index){
    const m=row?.metrics||{};
    return JSON.stringify([row?.displayPosition,index,row?.title,row?.status,row?.sourceType,!!row?.isExclusive,!!row?.exclusiveBlocked,row?.planText,row?.feedName,row?.category,row?.error,row?.stage,row?.provider,row?.model,m.aiQueueWaitMs,m.textElapsedMs,m.textTokensPerSec,m.pronunciationElapsedMs,m.ttsQueueWaitMs,m.ttsElapsedMs,m.audioDurationSec,m.totalElapsedMs,m.ttsRealtimeFactor,m.ttsEngine,m.ttsDevice,m.ttsVariant,m.ttsGpuName,m.ttsGpuVramMb,m.ttsRetryCount,m.ttsStallMs,m.ttsProgressCount,m.ttsLastError,m.pipelineMode,typeColor(row)]);
  }
