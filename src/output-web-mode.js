'use strict';
(function(){
  const mode=window.__GEC_OUTPUT_WEB_MODE__||{},monitor=!!mode.monitor,bar=document.getElementById('monitorBar'),btn=document.getElementById('monitorMute'),clock=document.getElementById('monitorClock'),modeText=document.getElementById('monitorMode'),dot=document.getElementById('monitorLiveDot');
  ...
  let muted=monitor,lastProgress={currentSec:0,durationSec:0};
  ...
  function applyAudio(){for(const el of media())el.muted=muted;if(btn)btn.textContent=muted?'🔇 Audio monitor':'🔊 Audio monitor';}
  if(monitor&&bar){bar.classList.remove('hidden');document.body.classList.add('monitor-mode');applyAudio();btn.onclick=()=>{muted=!muted;applyAudio();if(!muted){for(const el of media())if(el.paused&&el.src)el.play().catch(()=>{});}};}
