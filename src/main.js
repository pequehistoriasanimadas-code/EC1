...
function enrichDesign(raw={}){return{...raw,verticalVideoBackgroundUrl:fileUrl(raw.verticalVideoBackground),musicUrl:fileUrl(raw.musicFile),customFonts:customFontFaces()};}
...
ipcMain.handle('output:open',()=>{createOutputWindow(true);return{ok:true,state:{...outputState}};});ipcMain.handle('output:close',async()=>{if(!outputReady())return{ok:true,alreadyClosed:true,state:{...outputState}};outputWindow.hide();setOutputState({open:true,visible:false});return{ok:true,hidden:true,state:{...outputState}};});ipcMain.handle('output:status',()=>({...outputState,lan:outputLan?.status?.()||null}));ipcMain.handle('output:lanStatus',()=>outputLan?.status?.()||{enabled:false,error:'Servidor LAN no iniciado'});ipcMain.handle('output:lanEnsure',async()=>outputLan?outputLan.ensureRunning():{enabled:false,error:'Servidor LAN no iniciado'});ipcMain.handle('output:lanConfigure',async(_,p={})=>outputLan?outputLan.configure(p):{enabled:false,error:'Servidor LAN no iniciado'});ipcMain.handle('output:manualSend',async(_,p)=>{const a=automation.getState();if(a.emission.running){const r=await dialog.showMessageBox(controlWindow,{type:'warning',buttons:['Cancelar','Emitir ahora'],defaultId:0,cancelId:0,title:'Emisión automática activa',message:'La emisión automática está activa.',detail:'Si continúas, la emisión automática se pausará y esta nota tomará el Output.'});if(r.response!==1)return{ok:false,cancelled:true};automation.interruptForManual();}const ok=deliverToOutput({...p,kind:'news'},'manual',true);return{ok,source:'manual'};});ipcMain.on('output:control',(_,action)=>controlOutput(action));ipcMain.on('output:playback',(_,event)=>{try{outputLan?.updateMasterPlayback(event);}catch{}automation.outputPlayback(event);if(event?.source==='manual'&&event?.type==='ended')setOutputState({source:'manual'});});ipcMain.on('output:designPreview',(_,design)=>{const merged=enrichDesign({...settingsStore.load().visual.output,...design});if(outputReady())outputWindow.webContents.send('output:design',merged);try{outputLan?.publishDesign(merged);}catch{}if(outputReady()&&design?.format&&design.format!==outputState.format)applyOutputWindowFormat(design.format,true);});
