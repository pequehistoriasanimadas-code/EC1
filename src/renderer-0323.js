...
(function installGec0323Ui(){
  ...
  async function refreshPronunciation0323(){
    ...
    try{
      ...
      const badge=$('#pronunciationLearningCount');if(badge)badge.textContent=`${repl} ajuste${repl===1?'':'s'} · ${neg} verificadas`;
      const sum=$('#ec0323LearningSummary');if(sum)sum.textContent=`${repl} ajustes · ${manual} manual${manual===1?'':'es'} · ${neg} verificadas`;
      ...
      learningList=Array.isArray(p?.learningList)?p.learningList:[];renderLearningList();
