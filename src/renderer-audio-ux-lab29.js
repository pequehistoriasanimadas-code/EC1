    ...
    try{
      ...
      qa('#v2ReferenceList .v2-reference-row[data-id]').forEach(row=>{
        ...
        if(!row.querySelector('[data-play-reference]')){
          const play=document.createElement('button');play.type='button';play.className='dark compact v2-reference-play';play.dataset.playReference=id;
          play.title=`Reproducir referencia ${v?.name||''}`.trim();play.setAttribute('aria-label',play.title);play.innerHTML=icon('play');actions.prepend(play);
        ...
        if(del){del.classList.add('v2-reference-delete');del.title='Eliminar voz de referencia';del.setAttribute('aria-label','Eliminar voz de referencia');if(!del.dataset.iconified){del.dataset.iconified='1';del.innerHTML=icon('trash');}}
      });
