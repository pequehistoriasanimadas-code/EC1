...
class CustomVoiceManager{
  constructor({resourcesDir,dataDir}){
    ...
    this.tool=path.join(__dirname,'..','voice_tools.py');
  ...
  list(){const items=Array.isArray(safeJson(this.manifestFile,[]))?safeJson(this.manifestFile,[]):[];return items.filter(x=>x&&x.id&&x.file&&fs.existsSync(x.file)).map(x=>({...x,name:cleanName(x.name)||x.id}));}
  ...
  async importPt(src,name=''){
    ...
    const id=`ecv_${crypto.createHash('sha1').update(`${display}|${Date.now()}|${Math.random()}`).digest('hex').slice(0,12)}`,dest=path.join(this.customDir,`${id}.npz`);
    await this.run(['import-pt','--input',file,'--official',this.official,'--output',dest]);
    const item={id,name:display,file:dest,source:'Kokoro Voice Designer (.pt)',createdAt:new Date().toISOString()};existing.push(item);this.save(existing);try{await this.rebuild();}catch(e){this.save(existing.filter(x=>x.id!==id));try{fs.rmSync(dest,{force:true});}catch{}throw e;}return item;
  }
  ...
  async remove(id){const items=this.list(),item=items.find(x=>x.id===id);if(!item)throw new Error('Voz personalizada no encontrada');this.save(items.filter(x=>x.id!==id));try{fs.rmSync(item.file,{force:true});}catch{}await this.rebuild();return item;}
  exportFile(id,dest){const item=this.list().find(x=>x.id===id);if(!item)throw new Error('Voz personalizada no encontrada');fs.copyFileSync(item.file,dest);return{ok:true,path:dest,name:item.name};}
