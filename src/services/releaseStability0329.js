...
const profileGeneration=()=>Number(global.__ec0329ActiveProfileGeneration)||0;
function bumpProfileGeneration(){global.__ec0329ActiveProfileGeneration=profileGeneration()+1;return global.__ec0329ActiveProfileGeneration;}
...
function installProfileLimit(){
  ...
  const create=p.create,duplicate=p.duplicate,status=p.status,activate=p.activate;
  ...
  p.activate=function(id){const before=this.activeId();const result=activate.call(this,id);if(before!==this.activeId())bumpProfileGeneration();return result;};
...
function installAutomationResilience(){
  ...
  const process=p.process;if(typeof process==='function')p.process=async function(...args){const m=global.__ec0329ProfileManager,startId=m?.activeId?.()||'',startGeneration=profileGeneration();const out=await process.apply(this,args);const now=global.__ec0329ProfileManager;if(startId&&now&&(startId!==now.activeId?.()||startGeneration!==profileGeneration())){const e=new Error('El trabajo pertenece al perfil anterior');e.code='PROCESSING_CANCELLED';throw e;}return out;};
  const processDocument=p.processDocument;if(typeof processDocument==='function')p.processDocument=async function(...args){const m=global.__ec0329ProfileManager,startId=m?.activeId?.()||'',startGeneration=profileGeneration();const out=await processDocument.apply(this,args);const now=global.__ec0329ProfileManager;if(startId&&now&&(startId!==now.activeId?.()||startGeneration!==profileGeneration())){const e=new Error('El documento pertenece al perfil anterior');e.code='PROCESSING_CANCELLED';throw e;}return out;};
...
module.exports={installReleaseStability0329,MAX_PROFILES,importAddCount,profileLimitError,profileGeneration,bumpProfileGeneration};
