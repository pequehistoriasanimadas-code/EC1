...
class CannedManager{
  constructor(){this.signature='';this.bag=[];this.lastPath='';}
  ...
  list(folder){
    ...
    for(const e of entries){
      ...
      try{const st=fs.statSync(full);files.push({name:e.name,path:full,url:pathToFileURL(full).href,sizeBytes:st.size,sizeMB:Number((st.size/1048576).toFixed(1)),mtimeMs:st.mtimeMs,ext});}catch{}
    }
  ...
  ensureBag(folder){
    ...
    const currentPaths=new Set(scan.files.map(x=>x.path));this.bag=this.bag.filter(x=>currentPaths.has(x.path));
    if(!this.bag.length){
  ...
  peek(folder){
    const scan=this.ensureBag(folder);const item=this.bag[0];
    return item?{...item,remainingInCycle:Math.max(0,this.bag.length-1),total:scan.files.length}:null;
  }
  pick(folder){
    const scan=this.ensureBag(folder);const item=this.bag.shift();this.lastPath=item.path;
