...
class AutomationEngine extends Automation0324{
  ...
  reserveKnownExclusives(items,s){
    this.exclusiveReserve=Array.isArray(this.exclusiveReserve)?this.exclusiveReserve:[];this.exclusiveReserveKeys=this.exclusiveReserveKeys instanceof Set?this.exclusiveReserveKeys:new Set();if(this.schedulerState(s).due)return;const max=clamp(s?.automation?.exclusiveReserveMax,1,30,10);if(this.exclusiveReserve.length>=max)return;
  ...
  async producer(epoch){
    while(this.processingRunning&&epoch===this.processingEpoch){try{
      ...
      const s=this.getSettings(),target=Math.max(1,Math.min(30,Number(s.automation?.bufferReady)||15)),readyCount=this.readyItems().length,sched=this.schedulerState(s),needDueExclusive=this.needsDueExclusive(s),desired=target+(needDueExclusive?1:0),voiceBacklog=this.gpuStageQueue.filter(x=>x.kind==='voice').length+(this.gpuStageCurrent==='voice'?1:0),maxWorkers=voiceBacklog>=2?1:2;
      ...
      if(sched.due&&needDueExclusive&&this.exclusiveReserve.length){
        const reserved=this.takeReservedExclusive(s);if(reserved){this.lastNoRssAt=0;this.processingNotice=`Turno de exclusiva cumplido · preparando una de ${this.exclusiveReserve.length+1} reservada(s).`;this.launchCandidate(reserved.story,s,epoch,{forceExclusiveDue:true,reservedArticle:reserved.article,fromReserve:true});await wait(100);continue;}
      }
      let candidate=this.candidateFrom(this.cachedItems,s,{publicOnly:!sched.due});if(!candidate&&Date.now()-this.lastFeedFetchAt>15000){await this.refreshFeedCache(s,true);this.reserveKnownExclusives(this.cachedItems,s);candidate=this.candidateFrom(this.cachedItems,s,{publicOnly:!sched.due});}
      if(!candidate){if(!this.lastNoRssAt)this.lastNoRssAt=Date.now();this.processingNotice=!sched.due?`Buscando noticias sin candado · faltan ${sched.nonExclusiveNeeded} pública(s) para habilitar la próxima exclusiva · ${this.exclusiveReserve.length} reservada(s).`:'No hay noticias nuevas elegibles; esperando actualización de las fuentes.';this.state();this.kickDocumentWorker();await wait(1800);continue;}
      this.lastNoRssAt=0;this.processingNotice=!sched.due?`Priorizando noticia sin candado · ${this.exclusiveReserve.length} exclusiva(s) reservada(s).`:`Preparando reserva: ${readyCount}/${target} listas · pipeline escalonado.`;this.launchCandidate(candidate,s,epoch,{forceExclusiveDue:sched.due&&this.knownExclusive(candidate,s)});await wait(100);
