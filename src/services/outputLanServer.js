...
class OutputLanServer{
  ...
  allow(req,url){if(isLoopback(req.socket?.remoteAddress))return true;if(!this.config.enabled)return false;return String(url.searchParams.get('k')||'')===this.config.key;}
  handle(req,res){
