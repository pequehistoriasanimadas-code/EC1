...
class SettingsStore{
  ...
  load(){
    ...
    const o=data.visual.output,legacyFont=String(o.fontFamily||'Arial');for(const key of ['titleFontFamily','summaryFontFamily','categoryFontFamily','dateFontFamily','exclusiveFontFamily'])if(!o[key])o[key]=legacyFont;if(!o.dateColor)o.dateColor=o.summaryColor||'#F3F3F3';const legacyTransitionOff=raw?.visual?.output?.transitionEnabled===false;o.transitionType=legacyTransitionOff||o.transitionType==='none'?'none':'fade';o.transitionEnabled=o.transitionType!=='none';
