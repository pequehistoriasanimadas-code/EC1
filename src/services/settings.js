...
class SettingsStore{
  ...
  defaults(){return{
    ...
    tts:{voice:'ef_dora',speed:1.0,resourceMode:'safe_streaming',performanceThreads:6,pronunciationSmart:true,pronunciationClaudeVerify:true,pronunciationMaxSeconds:15,persistent:true,persistentIdleMinutes:5,autoTune:true,autoTuned:false},
  ...
  save(settings){
    fs.mkdirSync(this.baseDir,{recursive:true});const tmp=`${this.file}.tmp`,feeds=(Array.isArray(settings?.rssFeeds)?settings.rssFeeds:[]).map(f=>{const{publisherName,publisherWeb,partialCtaEnabled,partialCtaTemplate,...clean}=f||{};return clean;}),payload={...settings,rssFeeds:feeds,rssPartialClose:{enabled:settings?.rssPartialClose?.enabled!==false,template:String(settings?.rssPartialClose?.template||'Para más información, visita {web}.').trim()||'Para más información, visita {web}.'}};fs.writeFileSync(tmp,JSON.stringify(payload,null,2),'utf8');if(fs.existsSync(this.file)){try{fs.copyFileSync(this.file,this.backupFile);}catch{}}try{fs.renameSync(tmp,this.file);}catch{fs.copyFileSync(tmp,this.file);try{fs.rmSync(tmp,{force:true});}catch{}}
  }
