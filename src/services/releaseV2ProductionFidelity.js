...
const PROFILE_VERSION='2.0-lab.22';
...
function buildProfile(settings={},payload={}){
  ...
  const q=tts.engineParams?.qwen3tts||{},ch={...(tts.engineParams?.chatterbox||{}),variant:'latam'},localConfig=normalizeLocalConfig(payload?.localResult?.recommendedConfig||settings?.ai?.localTunedConfig||{});
  ...
  const voiceIdentity=engine==='chatterbox'?String(tts.referenceVoiceId||'')+'|latam':identity.fingerprint||identity.id||identity.name||String(tts.referenceVoiceId||'')||engine;
...
function compatibility(settings={},profile=null){
  ...
  if(profile.pipeline?.validated!==true)return{ok:false,reason:'perfil pendiente de revalidación lab.22'};
  if(profile.pipeline?.mode==='gpu-coordinated'&&profile.pipeline?.coordinatedValidated!==true)return{ok:false,reason:'GPU coordinada no fue validada con la prueba secuencial lab.22'};
...
async function preflightAutomation(engine){
  ...
  if(!profile&&s.optimization0321){const e=new Error('Existe una optimización anterior, pero falta el perfil de producción lab.22. Ejecuta Optimizar GEC una vez.');e.code='PRODUCTION_PROFILE_REOPTIMIZE_REQUIRED';throw e;}
