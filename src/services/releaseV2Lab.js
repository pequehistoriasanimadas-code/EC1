...
function normalizeProfileTts(raw={}){const out={...raw};out.engine=VALID_ENGINES.has(String(out.engine||''))?String(out.engine):'kokoro';out.style=VALID_STYLES.has(String(out.style||''))?String(out.style):'news';out.referenceVoiceId=String(out.referenceVoiceId||'');out.fallbackToKokoro=out.fallbackToKokoro!==false;out.engineParams=out.engineParams&&typeof out.engineParams==='object'?out.engineParams:{};out.engineParams.chatterbox={exaggeration:Number(out.engineParams.chatterbox?.exaggeration??0.45),cfgWeight:Number(out.engineParams.chatterbox?.cfgWeight??0.40),temperature:Number(out.engineParams.chatterbox?.temperature??0.8),speed:Math.max(0.85,Math.min(1.25,Number(out.engineParams.chatterbox?.speed??1)))};out.engineParams.qwen3tts={temperature:Number(out.engineParams.qwen3tts?.temperature??0.78),speed:Math.max(0.85,Math.min(1.25,Number(out.engineParams.qwen3tts?.speed??1))),voiceMode:String(out.engineParams.qwen3tts?.voiceMode||'reference')==='finetuned'?'finetuned':'reference',fineTunedModelId:String(out.engineParams.qwen3tts?.fineTunedModelId||'')};return out;}
