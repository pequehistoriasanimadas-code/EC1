...
function normalizeProfileTts(raw={}){const out={...raw};out.engine=VALID_ENGINES.has(String(out.engine||''))?String(out.engine):'kokoro';out.style=VALID_STYLES.has(String(out.style||''))?String(out.style):'news';out.referenceVoiceId=String(out.referenceVoiceId||'');out.fallbackToKokoro=out.fallbackToKokoro!==false;out.engineParams=out.engineParams&&typeof out.engineParams==='object'?out.engineParams:{};out.engineParams.chatterbox={exaggeration:Number(out.engineParams.chatterbox?.exaggeration??0.45),cfgWeight:Number(out.engineParams.chatterbox?.cfgWeight??0.40),temperature:Number(out.engineParams.chatterbox?.temperature??0.8),speed:Math.max(0.85,Math.min(1.25,Number(out.engineParams.chatterbox?.speed??1))),variant:['latam','multilingual'].includes(String(out.engineParams.chatterbox?.variant||''))?String(out.engineParams.chatterbox.variant):'latam'};out.engineParams.qwen3tts={temperature:Number(out.engineParams.qwen3tts?.temperature??0.78),speed:Math.max(0.85,Math.min(1.25,Number(out.engineParams.qwen3tts?.speed??1))),voiceMode:String(out.engineParams.qwen3tts?.voiceMode||'reference')==='finetuned'?'finetuned':'reference',fineTunedModelId:String(out.engineParams.qwen3tts?.fineTunedModelId||'')};return out;}
...
function optimizationKey(tts={}){const engine=String(tts.engine||'kokoro');return engine==='chatterbox'?`chatterbox:${String(tts.engineParams?.chatterbox?.variant||'latam')}`:engine;}
...
function installSettings(){const p=SettingsStore.prototype;if(p.__gecV2TtsSettings)return;Object.defineProperty(p,'__gecV2TtsSettings',{value:true});const baseDefaults=p.defaults,baseLoad=p.load,baseSave=p.save;
  ...
  p.load=function(){const s=baseLoad.call(this);s.tts=s.tts||{};const rawVariant=String(s.tts?.engineParams?.chatterbox?.variant||'');Object.assign(s.tts,normalizeProfileTts(s.tts));s.tts.engineOptimizations=s.tts.engineOptimizations&&typeof s.tts.engineOptimizations==='object'?s.tts.engineOptimizations:{};
    if(s.tts.engineOptimizations.chatterbox&&!s.tts.engineOptimizations['chatterbox:multilingual']){s.tts.engineOptimizations['chatterbox:multilingual']=s.tts.engineOptimizations.chatterbox;delete s.tts.engineOptimizations.chatterbox;}
    ...
    if(engine==='chatterbox'&&current&&!current.ttsVariant){current.ttsVariant=rawVariant||'multilingual';const legacyKey='chatterbox:'+current.ttsVariant;if(!s.tts.engineOptimizations[legacyKey])s.tts.engineOptimizations[legacyKey]=captureOptimization(s);}
    const currentKey=current?.ttsEngine==='chatterbox'?'chatterbox:'+String(current.ttsVariant||'multilingual'):String(current?.ttsEngine||'');
    if(current&&currentKey&&currentKey!==key)applyOptimization(s,s.tts.engineOptimizations[key]||null);
    return s;};
  p.save=function(settings){const s=settings&&typeof settings==='object'?settings:{};s.tts=s.tts||{};Object.assign(s.tts,normalizeProfileTts(s.tts));s.tts.engineOptimizations=s.tts.engineOptimizations&&typeof s.tts.engineOptimizations==='object'?s.tts.engineOptimizations:{};const engine=s.tts.engine||'kokoro',key=optimizationKey(s.tts);if(s.optimization0321){s.optimization0321={...s.optimization0321,ttsEngine:engine,ttsVariant:engine==='chatterbox'?s.tts.engineParams.chatterbox.variant:''};s.tts.engineOptimizations[key]=captureOptimization(s);}return baseSave.call(this,s);};
}
