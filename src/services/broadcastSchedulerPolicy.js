...
function rebalance(engine){
  ...
  if(st.rssSinceGenerated>=generatedEveryRss&&readyRss.length){const chosen=readyGen[0],firstRss=readyRss[0],target=q.indexOf(firstRss);moveItem(q,chosen,target);chosen.schedulerPinned=true;return;}
  if(st.rssSinceGenerated<generatedEveryRss&&readyRss.length){for(const g of readyGen)g.schedulerPinned=false;const ordered=[...q.filter(x=>!(x.sourceType==='generated'&&x.status==='LISTA')),...readyGen];q.splice(0,q.length,...ordered);}
}
...
function productionProfile(engine){
  const st=stateFor(engine),live=[];for(const x of engine.queue||[]){if(!isNews(x))continue;const s=sampleFromItem(x);if(s)live.push(s);}const samples=[...st.samples,...live].slice(-24);if(!samples.length)return{rate:.6,avgAudio:50,count:0};
...
function adRecoverySeconds(engine,s){const folder=String(s?.canned?.adsFolder||'').trim();if(!folder||s?.canned?.insertAdAfterContent===false)return 0;try{return Number(engine.ads?.peek?.(folder)?.durationSec)||0;}catch{return 0;}}
...
function plannedRows(engine,s,realRows){
  ...
  if(engine.cannedRequested){reason='manual';after=0;}else if(progress.due){reason='scheduled';after=0;}else if(interval>0&&progress.nextIn!=null&&qualified.length>=progress.nextIn){reason='scheduled';after=Math.max(0,progress.nextIn);}else if(c.emergency!==false&&qualified.length===0){reason='emergency';after=0;}if(!reason)return rows;
  ...
  const duration=Number(media.durationSec)||0,recoveryText=reason==='scheduled'&&c.adaptiveDuration!==false&&duration>0?` · ${Math.round(duration/60*10)/10} min de recuperación`:'';const planText=reason==='scheduled'?`Después de ${interval} noticia${interval===1?'':'s'}${recoveryText}`:reason==='emergency'?'Recuperación por autonomía baja':'Próximo en emisión';rows.push({title:media.name,status:'PROGRAMADO',sourceType:'content',planned:true,stage:'',planAfter:after,planText,planReason:reason,durationSec:duration,recoveryTargetSec:Number.isFinite(target)?target:0});
  if(c.insertAdAfterContent!==false&&String(c.adsFolder||'').trim()){let ad=null;try{ad=engine.ads?.peek(c.adsFolder||'');}catch{}if(ad)rows.push({title:ad.name,status:'PROGRAMADO',sourceType:'ad',planned:true,stage:'',planAfter:after,planText:'Después del contenido',planReason:reason,durationSec:Number(ad.durationSec)||0});}return rows;
}
...
function installBroadcastSchedulerPolicy(){
  ...
  proto.addEmissionHistory=function(type,title,status='EMITIDA',extra={}){if(status==='EMITIDA'&&(type==='rss'||type==='generated')){const st=stateFor(this),sample=sampleFromItem(this.currentItem,extra);if(sample){st.samples.push(sample);st.samples=st.samples.slice(-20);}if(type==='rss')st.rssSinceGenerated++;else st.rssSinceGenerated=0;}return originalAddHistory.call(this,type,title,status,extra);};
  ...
  proto.__ecRecoveryTargetSec=function(s,reason){return recoveryTargetSeconds(this,s||this.getSettings?.()||{},reason);};proto.__ecProductionProfile=function(){return productionProfile(this);};
