...
class LocalRuntime{
  constructor({resourcesDir,dataDir,onEvent=()=>{}}){
    ...
    this.server=null;this.serverExeCache='';this.port=8766;this.startingPromise=null;this.downloadPromise=null;this.lastDownloadProgressAt=0;this.lastDownloadPercent=-1;this.idleTimer=null;this.idleDeadline=0;this.resourceMode='safe_streaming';this.generationTail=Promise.resolve();this.generationActive=false;fs.mkdirSync(this.modelDir,{recursive:true});
  }
  ...
  emitDownloadProgress(done,total,force=false){
    const now=Date.now(),percent=total?Math.max(0,Math.min(100,Math.floor(done*100/total))):0,percentChanged=total?percent!==this.lastDownloadPercent:true,timeReady=now-this.lastDownloadProgressAt>=250;
    if(!force&&(!percentChanged||!timeReady))return;
    this.lastDownloadProgressAt=now;this.lastDownloadPercent=percent;this.onEvent({type:'model-download',done,total,percent});
  }
  async downloadModel(){
    if(this.modelReady())return{ok:true,path:this.modelPath,already:true};if(this.downloadPromise)return this.downloadPromise;
    this.lastDownloadProgressAt=0;this.lastDownloadPercent=-1;
    this.downloadPromise=(async()=>{fs.mkdirSync(this.modelDir,{recursive:true});const tmp=this.modelPath+'.part';try{if(fs.existsSync(tmp))fs.rmSync(tmp,{force:true});}catch{}
      try{const res=await fetch(MODEL_URL,{redirect:'follow',headers:{'user-agent':'EC-Automatic-News/0.3.14'},signal:AbortSignal.timeout(2*60*60*1000)});if(!res.ok)throw new Error(`Descarga Qwen HTTP ${res.status}`);if(!res.body)throw new Error('El servidor no devolvió datos del modelo Qwen');const total=Number(res.headers.get('content-length')||0);let done=0;const out=fs.createWriteStream(tmp,{flags:'w'}),stream=Readable.fromWeb(res.body);stream.on('data',chunk=>{done+=chunk.length;this.emitDownloadProgress(done,total,false);});await new Promise((resolve,reject)=>{stream.pipe(out);out.on('finish',resolve);out.on('error',reject);stream.on('error',reject);});this.emitDownloadProgress(done,total,true);const size=fs.existsSync(tmp)?fs.statSync(tmp).size:0;if(size<MIN_MODEL_BYTES)throw new Error(`Descarga Qwen incompleta (${Math.round(size/1048576)} MB)`);if(fs.existsSync(this.modelPath))fs.rmSync(this.modelPath,{force:true});fs.renameSync(tmp,this.modelPath);this.onEvent({type:'model-downloaded',path:this.modelPath,size});return{ok:true,path:this.modelPath,size};}
      catch(e){try{if(fs.existsSync(tmp))fs.rmSync(tmp,{force:true});}catch{}this.onEvent({type:'model-download-error',message:e.message||String(e)});throw e;}})();try{return await this.downloadPromise;}finally{this.downloadPromise=null;}
  }
