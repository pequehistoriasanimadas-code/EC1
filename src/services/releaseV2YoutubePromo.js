...
let autoTimer=null,commandTail=Promise.resolve();
...
function stale(row,now=Date.now()){const at=Date.parse(row?.lastCheckedAt||row?.updatedAt||'');return!(at>0)||now-at>=core.TTL_MS;}
function cachedVideoUsable(ctx,row){return!!(row?.videoId&&String(row.title||'').trim()&&thumbnailPath(ctx,row)&&fs.existsSync(thumbnailPath(ctx,row)));}
async function refreshIds(base,profileId,ids,{force=false,baseLoad,baseSave,store}={}){
  ...
  let settings=normalizeSettings(baseLoad.call(store)),promo=settings.canned.youtubePromo,unique=[...new Set((ids||[]).map(String).filter(Boolean))];const now=Date.now();if(!force)unique=unique.filter(id=>stale(promo.videos[id],now));if(!unique.length)return{updated:0,skipped:true};
...
async function handleCommand(base,cmd,{baseLoad,baseSave}){
  ...
  try{
    ...
    if(action==='unlink'){const key=String(cmd.contentKey||'');if(key&&promo.links[key]){delete promo.links[key];settings.canned.youtubePromo=promo;if(!activeStill(base,profileId))throw new Error('El perfil cambió antes de guardar');baseSave.call(store,cleanRuntimeFields(settings));}mark(base,profileId,{message:'Vínculo de YouTube eliminado.'});return;}
    if(action==='link'){
      ...
      const existing=promo.videos[videoId];if(!cachedVideoUsable(ctx,existing))await refreshIds(base,profileId,[videoId],{force:true,baseLoad,baseSave,store});
      settings=normalizeSettings(baseLoad.call(store));promo=settings.canned.youtubePromo;if(!activeStill(base,profileId))throw new Error('El perfil cambió antes de vincular el video');promo.links[contentKey]={videoId,fileName:String(cmd.fileName||''),sizeBytes:Math.max(0,Number(cmd.sizeBytes)||0),linkedAt:new Date().toISOString()};settings.canned.youtubePromo=promo;baseSave.call(store,cleanRuntimeFields(settings));mark(base,profileId,{message:existing?'Vínculo reutilizado sin repetir la consulta.':'Video de YouTube vinculado correctamente.'});return;
    }
...
function contentFolder(folder,s){const raw=String(folder||'');if(!raw)return false;const c=String(s?.canned?.folder||''),a=String(s?.canned?.adsFolder||'');try{const f=path.resolve(raw),cf=c?path.resolve(c):'',af=a?path.resolve(a):'';if(af&&f===af&&(!cf||cf!==f))return false;return!!cf&&f===cf;}catch{return false;}}
...
function installSettingsBridge(){const p=SettingsStore.prototype;if(p.__ecLab27YoutubePromo)return;Object.defineProperty(p,'__ecLab27YoutubePromo',{value:true});const baseLoad=p.load,baseSave=p.save;
  ...
  p.save=function(settings,...args){const base=this.baseDir||dataRoot(),cmd=clone(settings?.__youtubePromoCommand),clean=cleanRuntimeFields(normalizeSettings(clone(settings)||{})),r=baseSave.call(this,clean,...args);if(cmd&&cmd.type)setTimeout(()=>{commandTail=commandTail.then(()=>handleCommand(base,cmd,{baseLoad,baseSave})).catch(()=>{});},0);return r;};
...
function installAutomationSnapshot(){const p=BaseAutomationEngine.prototype;if(p.__ecLab27YoutubePromo)return;const basePlay=p.playCanned;if(typeof basePlay!=='function')return;Object.defineProperty(p,'__ecLab27YoutubePromo',{value:true});p.playCanned=async function(...args){const originalSend=this.sendAutomaticOutput;if(typeof originalSend!=='function')return basePlay.apply(this,args);this.sendAutomaticOutput=(payload)=>{let next=payload;if(String(payload?.mediaRole||'')==='content'&&this.currentCanned?.youtubePromo)next={...payload,youtubePromo:clone(this.currentCanned.youtubePromo)};else if(String(payload?.mediaRole||'')==='ad')next={...payload,youtubePromo:null};return originalSend(next);};try{return await basePlay.apply(this,args);}finally{this.sendAutomaticOutput=originalSend;}};}
...
function autoRefresh(){const base=dataRoot(),profileId=activeContext(base).profileId;if(!profileId||runtime.busy)return;const baseLoad=global.__ecLab27YoutubeBaseLoad,baseSave=global.__ecLab27YoutubeBaseSave;if(typeof baseLoad!=='function'||typeof baseSave!=='function')return;const store=storeFor(base);if(!keyStatus(base,store))return;commandTail=commandTail.then(()=>handleCommand(base,{type:'auto-refresh',requestId:`auto-${Date.now()}`},{baseLoad,baseSave})).catch(()=>{});}
function installReleaseV2YoutubePromo(){if(installed)return;installed=true;installSettingsBridge();installCannedDecoration();installAutomationSnapshot();installWindowInjection();app.whenReady().then(()=>{setTimeout(autoRefresh,9000);autoTimer=setInterval(autoRefresh,60*60*1000);}).catch(()=>{});app.on('before-quit',()=>{if(autoTimer)clearInterval(autoTimer);});}

module.exports={installReleaseV2YoutubePromo,decorateItem,normalizeSettings,handleCommand,stale};
