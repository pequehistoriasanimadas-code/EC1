...
function contentFingerprint(file){try{loadIndex();const st=fs.statSync(file),resolved=path.resolve(file),key=resolved.toLocaleLowerCase('es'),signature=`${st.size}|${Math.round(st.mtimeMs)}`,cached=index.entries[key];if(cached?.signature===signature&&/^[0-9a-f]{64}$/i.test(cached.hash||'')){cached.lastSeenAt=Date.now();return cached.hash;}const hash=streamHashSync(file);index.entries[key]={signature,hash,lastSeenAt:Date.now()};scheduleSave();return hash;}catch{return'';}}
