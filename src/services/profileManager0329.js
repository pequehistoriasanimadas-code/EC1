...
class ProfileManager0329{
  ...
  create({name,color,defaults}){const clean=safeName(name),c=safeColor(color);if(!clean)throw new Error('Escribe un nombre para el perfil');if(!c)throw new Error('Selecciona un color válido');if(this.registry.profiles.some(x=>sameName(x.name,clean)))throw new Error('Ya existe un perfil con ese nombre');const first=this.registry.profiles.length===0,id=crypto.randomUUID(),meta={id,name:clean,color:c,schemaVersion:PROFILE_SCHEMA_VERSION,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()};const base=splitSettings(defaults||{}).profilePart;base.rssFeeds=[];base.documents={...(base.documents||{}),processed:{}};ensureSourceIds(base);fs.mkdirSync(this.assetsDir(id),{recursive:true});atomicJson(path.join(this.profileDir(id),'profile.json'),meta);this.writeProfileSettings(id,base);atomicJson(this.historyFile(id),{emitted:[],automation:{}});atomicJson(this.cycleFile(id),{version:2,sources:{}});this.registry.profiles.push(meta);if(first)this.registry.activeProfileId=id;this.saveRegistry();return{...meta,active:first};}
