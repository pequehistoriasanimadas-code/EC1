...
(function installReleaseUx0329(){
  ...
  function polishDock(){
    const btn=q('#ec29ProfileButton'),name=q('#ec29ProfileName');if(!btn||!name)return;
    let dot=btn.querySelector('.ec29-current-dot');if(!dot){dot=document.createElement('span');dot.className='ec29-current-dot';btn.insertBefore(dot,name);}
    const color=profileColor(name);dot.style.background=color;btn.style.setProperty('--profile-color',color);
    const arrow=btn.querySelector('.ec29-profile-arrow');if(arrow)arrow.textContent='⌃';
  }
  ...
  const name=q('#ec29ProfileName');if(name)new MutationObserver(polishDock).observe(name,{attributes:true,childList:true,characterData:true,subtree:true});
  inspect();installDirtyTracking();setTimeout(installOptimizerObserver,450);
})();
