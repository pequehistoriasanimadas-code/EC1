...
(function installNewsSync0324(){
  ...
  function paintImage(story,image,isFallback=false){if(!image)return;for(const el of matchingCards(story)){const thumb=el.querySelector('.thumb');if(!thumb)continue;thumb.style.backgroundImage=`url(${JSON.stringify(image)})`;thumb.classList.remove('image-resolving','image-missing');thumb.classList.add(isFallback?'image-fallback':'image-resolved');}}
  function markNoArticleImage(story){const fallback=String(settings?.visual?.fallbackImageUrl||'').trim();if(fallback){paintImage(story,fallback,true);return;}for(const el of matchingCards(story)){const thumb=el.querySelector('.thumb');thumb?.classList.remove('image-resolving');thumb?.classList.add('image-missing');}}
  ...
  async function hydrate(story){const link=String(story?.link||'').trim(),cacheKey=baseKeyOf(story)||link;try{const cached=articleCache.get(cacheKey);let article=cached&&Date.now()-cached.at<RETRY_MS?cached.article:null;if(!article){article=await window.ECAPI.fetchArticle(link);remember(cacheKey,article||{});}if(article?.image){story.image=String(article.image);paintImage(story,story.image);}else markNoArticleImage(story);if(article?.pubDate&&!story.pubDate)story.pubDate=article.pubDate;if(article?.isExclusive){story.isExclusive=true;story.accessStatus=article.access?.status||'SUBSCRIBER_ONLY';}patchBadges(story);
    }catch{markNoArticleImage(story);}finally{queued.delete(cacheKey);active--;pump();}}
  function pump(){while(active<CONCURRENCY&&queue.length){const s=queue.shift();if(!s)continue;active++;hydrate(s);}}
  function scheduleHydration(list=visibleStories()){for(const story of list.slice(0,MAX_HYDRATE_PER_PASS)){const k=baseKeyOf(story)||String(story.link||'');if(!k||story.image||queued.has(k))continue;const cached=articleCache.get(k);if(cached&&Date.now()-cached.at<RETRY_MS){const a=cached.article||{};if(a.image){story.image=a.image;paintImage(story,a.image);}else markNoArticleImage(story);if(a.pubDate&&!story.pubDate)story.pubDate=a.pubDate;if(a.isExclusive)story.isExclusive=true;patchBadges(story);continue;}queued.add(k);queue.push(story);}pump();}
  function putLatest(key,value){if(!key)return;const old=statusMap.get(key);if(!old||Number(value?.updatedAt||0)>=Number(old?.updatedAt||0))statusMap.set(key,value);}
  function ingestState(s){lastState=s||lastState;for(const x of s?.newsStatuses||[]){if(!x?.storyKey&&!x?.baseKey)continue;putLatest(x.storyKey,x);putLatest(x.baseKey,x);const story=stories.find(st=>keyOf(st)===x.storyKey||baseKeyOf(st)===x.baseKey);if(story){if(x.isExclusive)story.isExclusive=true;patchBadges(story);}}while(statusMap.size>1000)statusMap.delete(statusMap.keys().next().value);}
