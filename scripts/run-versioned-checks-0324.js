...
let failed=false;
function run(script,version){const compat={...original,version};fs.writeFileSync(pkgFile,JSON.stringify(compat,null,2)+'\n','utf8');const r=spawnSync(process.execPath,[path.join(__dirname,script)],{cwd:root,stdio:'inherit'});if(r.status!==0)failed=true;}
try{
