...
app.whenReady().then(async()=>{let tmp='';try{
  ...
  assert(renderer.includes('v2InstallProgress')&&renderer.includes('Reintentar activación')&&renderer.includes("window.__GEC_V2LAB_RENDERER_RESPONSIVE__='lab20'"),'UX lab.23 no empaquetada');
  ...
  console.log('PACKAGED V2 HARDENING lab.23 OK · single-instance · EPERM retry · maintenance · transactional engines · clean PYTHONPATH · overlay/heartbeat');
  app.exit(0);
