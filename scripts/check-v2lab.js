...
assert.strictEqual(pkg.version,'2.0.0-lab.14','La build debe identificarse como 2.0.0-lab.14');
...
assert(renderer.includes("window.__GEC_V2LAB_RENDERER_RESPONSIVE__='lab14'")&&release.includes('await labRuntime().importFineTunedZip'),'Hardening de responsividad lab.14 incompleto');
...
assert(baseOptimizerUi.includes('ttsLabel0321')&&baseOptimizerUi.includes('ttsLabStatus')&&baseOptimizerUi.includes("version:'2.0-lab.14'")&&baseOptimizerUi.includes('ttsLabStop')&&baseOptimizerUi.includes('ttsLabValidateSelected'),'Optimizador base V2 debe prevalidar Qwen, liberar TTS antes de Qwen local y usar la GPU CUDA real');
...
console.log('check-v2lab: OK · lab.14 · UI responsiva · CUDA health async/cache · q fixed · import async · aislamiento Chatterbox/Qwen · GPU SWAP');
