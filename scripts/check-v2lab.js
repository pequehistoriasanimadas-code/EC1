...
assert.strictEqual(pkg.version,'2.0.0-lab.9','La build debe identificarse como 2.0.0-lab.9');
...
assert(preload.includes('ttsLabSelectEngine')&&preload.includes('ttsLabImportReference')&&preload.includes('ttsLabPrepareReference')&&preload.includes('ttsLabUpdateReference')&&preload.includes('ttsLabImportFineTunedModel')&&preload.includes('ttsLabStop')&&preload.includes('ttsLabWarmupSelected'),'Bridge TTS Lab incompleto');
...
assert(runtime.includes("CACHE_REVISION='lab9-r1'")&&runtime.includes('coldStartRealtimeFactor')&&runtime.includes('stableRealtimeFactor')&&runtime.includes('async warmup'),'Runtime debe versionar caché, separar arranque en frío y mantener warm-up explícito');
...
assert(baseOptimizerUi.includes('ttsLabel0321')&&baseOptimizerUi.includes('ttsLabStatus')&&baseOptimizerUi.includes("version:'2.0-lab.9'")&&baseOptimizerUi.includes('ttsLabStop'),'Optimizador base V2 debe nombrar el motor real, liberar TTS antes de Qwen y usar la GPU CUDA real');
...
assert(release.includes('installProcessingWarmup')&&release.includes('ttsRuntimeSignature')&&release.includes('CACHE_REVISION'),'Inicio de preparación debe calentar TTS e invalidar optimizaciones de runtime antiguas');
...
console.log('check-v2lab: OK · lab.9 · pronunciación fuera de GPU · warm-up/cache versionada · anti-starvation · watchdog/backpressure · progreso granular');
