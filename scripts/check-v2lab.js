...
assert.strictEqual(pkg.version,'2.0.0-lab.21','La build debe identificarse como 2.0.0-lab.21');
...
assert(boot.includes('canonicalLabBase')&&boot.includes('recoverNestedLabData')&&boot.includes('GEC_V2_TTS_LAB_ROOT')&&boot.includes("require('./bootstrap-0332')")&&boot.includes('releaseV2Optimization'),'V2 Lab debe usar una ruta canónica, recuperar datos anidados, heredar 0.3.32 e instalar guard de convivencia');
assert(pathGuard.includes("LAB_DIR_NAME='GEC V2 TTS Lab'")&&pathGuard.includes('while(isLabDir(base)&&isLabDir(path.dirname(base)))')&&pathGuard.includes('recoverNestedLabData'),'Guard de ruta V2 debe ser idempotente y recuperar perfiles de relaunch antiguos');
assert(mainSource.includes("process.env.GEC_V2_TTS_LAB_ROOT")&&profilePolicy.includes("process.env.GEC_V2_TTS_LAB_ROOT")&&release.includes("process.env.GEC_V2_TTS_LAB_ROOT"),'Main, perfiles y TTS deben compartir el mismo data root V2 canónico');
...
assert(worker.includes('QWEN_PERF_REVISION = 4')&&worker.includes('_timed_qwen_wrapper')&&worker.includes('gpu_telemetry')&&worker.includes('generate_qwen_finetuned_batch'),'Lab.20 debe instrumentar Qwen y soportar batching fine-tuned');
assert(runtime.includes('stageTimings:r.stage_timings')&&runtime.includes('gpuTelemetry:r.gpu_telemetry')&&runtime.includes("bottleneck=gpuAvg>90?'gpu-bound'"),'Lab.19 debe propagar telemetría y clasificar el cuello de botella');
assert(runtime.includes('sampleCount=5')&&runtime.includes('batchSampleCount=2')&&runtime.includes('subtalkerTopK:20')&&runtime.includes('batchVramHeadroomMb'),'Lab.20 debe comparar sampling con N=5 y batching con guardia VRAM');
assert(worker.includes('ctypes.WinDLL("nvml.dll")')&&worker.includes('code_predictor_steps')&&worker.includes('code_predictor_ms_per_step')&&worker.includes('kwargs["output_hidden_states"] = False'),'Lab.20 debe usar NVML, medir ms/paso y desactivar hidden states del predictor cuando corresponda');
assert(baseOptimizerUi.includes('QWEN3-TTS LAB.21 · SAMPLING (N=5)')&&baseOptimizerUi.includes('QWEN3-TTS LAB.21 · BATCHING (N=2 + perfil)')&&baseOptimizerUi.includes('DIAGNÓSTICO GANADOR LAB.21'),'Lab.20 debe mostrar sampling, batching y diagnóstico técnico');
...
assert(!worker.includes('time_stretch_preserve_pitch')&&!worker.includes('phase_vocoder')&&worker.includes('speed = 1.0'),'Lab.17 debe generar a velocidad natural sin time-stretch ni phase-vocoder');
...
assert(worker.includes('def cleanup_chatterbox_tail')&&worker.includes('post_silence_residual')&&worker.includes('chatterbox_tail_cleanup_ms'),'Hotfix lab.21 debe limpiar colas Chatterbox de forma conservadora y auditable');
assert(read('src/main.js').includes('const persisted=settingsStore.load()')&&read('src/renderer-actions.js').includes('queueDesignAutosave'),'Hotfix lab.21 debe persistir música, diseño y volúmenes por perfil');
...
assert(renderer.includes("engine==='qwen3tts'? ")&&renderer.includes('Chatterbox LatAm')&&!renderer.includes('Chatterbox Multilingual'),'Las tarjetas de referencia deben mostrar Qwen por separado y Chatterbox únicamente LatAm');
...
assert(renderer.includes("window.__GEC_V2LAB_RENDERER_RESPONSIVE__='lab20'")&&release.includes('await labRuntime().importFineTunedZip'),'Hardening de responsividad lab.21 incompleto');
...
assert(baseOptimizerUi.includes('ttsLabel0321')&&baseOptimizerUi.includes('ttsLabStatus')&&baseOptimizerUi.includes("version:'2.0-lab.21'")&&baseOptimizerUi.includes('ttsLabStop')&&baseOptimizerUi.includes('ttsLabValidateSelected'),'Optimizador base V2 debe prevalidar Qwen, liberar TTS antes de Qwen local y usar la GPU CUDA real');
...
console.log('check-v2lab: OK · lab.21 · production profile · tuned fidelity · stable voice chunks · UI responsiva · CUDA isolation');
