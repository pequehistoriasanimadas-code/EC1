...
assert.strictEqual(pkg.version,'2.0.0-lab.18','La build debe identificarse como 2.0.0-lab.18');
...
assert(baseOptimizerUi.includes('QWEN3-TTS LAB.18 · CANDIDATOS')&&baseOptimizerUi.includes('DIAGNÓSTICO GANADOR'),'Lab.18 debe mostrar el diagnóstico técnico comparativo');
...
assert(renderer.includes("window.__GEC_V2LAB_RENDERER_RESPONSIVE__='lab18'")&&release.includes('await labRuntime().importFineTunedZip'),'Hardening de responsividad lab.18 incompleto');
...
assert(baseOptimizerUi.includes('ttsLabel0321')&&baseOptimizerUi.includes('ttsLabStatus')&&baseOptimizerUi.includes("version:'2.0-lab.18'")&&baseOptimizerUi.includes('ttsLabStop')&&baseOptimizerUi.includes('ttsLabValidateSelected'),'Optimizador base V2 debe prevalidar Qwen, liberar TTS antes de Qwen local y usar la GPU CUDA real');
...
console.log('check-v2lab: OK · lab.18 · production profile · tuned fidelity · stable voice chunks · UI responsiva · CUDA isolation');
