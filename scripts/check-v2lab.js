...
assert.strictEqual(pkg.version,'2.0.0-lab.23','La build debe identificarse como 2.0.0-lab.23');
...
assert(baseOptimizerUi.includes('QWEN3-TTS LAB.23 · SAMPLING (N=5)')&&baseOptimizerUi.includes('QWEN3-TTS LAB.23 · BATCHING (N=2 + perfil)')&&baseOptimizerUi.includes('DIAGNÓSTICO GANADOR LAB.23'),'Lab.20 debe mostrar sampling, batching y diagnóstico técnico');
...
assert(worker.includes('def cleanup_chatterbox_tail')&&worker.includes('post_silence_residual')&&worker.includes('chatterbox_tail_cleanup_ms'),'Hotfix lab.23 debe limpiar colas Chatterbox de forma conservadora y auditable');
assert(read('src/main.js').includes('const persisted=settingsStore.load()')&&read('src/renderer-actions.js').includes('queueDesignAutosave'),'Hotfix lab.23 debe persistir música, diseño y volúmenes por perfil');
...
assert(renderer.includes("window.__GEC_V2LAB_RENDERER_RESPONSIVE__='lab20'")&&release.includes('await labRuntime().importFineTunedZip'),'Hardening de responsividad lab.23 incompleto');
...
assert(baseOptimizerUi.includes('ttsLabel0321')&&baseOptimizerUi.includes('ttsLabStatus')&&baseOptimizerUi.includes("version:'2.0-lab.23'")&&baseOptimizerUi.includes('ttsLabStop')&&baseOptimizerUi.includes('ttsLabValidateSelected'),'Optimizador base V2 debe prevalidar Qwen, liberar TTS antes de Qwen local y usar la GPU CUDA real');
