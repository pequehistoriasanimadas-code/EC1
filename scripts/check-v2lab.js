...
assert.strictEqual(pkg.version,'2.0.0-lab.20','La build debe identificarse como 2.0.0-lab.20');
...
assert(baseOptimizerUi.includes('QWEN3-TTS LAB.20 · SAMPLING (N=5)')&&baseOptimizerUi.includes('QWEN3-TTS LAB.20 · BATCHING (N=2 + perfil)')&&baseOptimizerUi.includes('DIAGNÓSTICO GANADOR LAB.20'),'Lab.20 debe mostrar sampling, batching y diagnóstico técnico');
...
assert(renderer.includes("window.__GEC_V2LAB_RENDERER_RESPONSIVE__='lab20'")&&release.includes('await labRuntime().importFineTunedZip'),'Hardening de responsividad lab.20 incompleto');
...
assert(baseOptimizerUi.includes('ttsLabel0321')&&baseOptimizerUi.includes('ttsLabStatus')&&baseOptimizerUi.includes("version:'2.0-lab.20'")&&baseOptimizerUi.includes('ttsLabStop')&&baseOptimizerUi.includes('ttsLabValidateSelected'),'Optimizador base V2 debe prevalidar Qwen, liberar TTS antes de Qwen local y usar la GPU CUDA real');
...
console.log('check-v2lab: OK · lab.20 · production profile · tuned fidelity · stable voice chunks · UI responsiva · CUDA isolation');
