...
assert.strictEqual(pkg.version,'2.0.0-lab.19','La build debe identificarse como 2.0.0-lab.19');
...
assert(worker.includes('QWEN_PERF_REVISION = 3')&&worker.includes('_timed_qwen_wrapper')&&worker.includes('gpu_telemetry')&&worker.includes('non_streaming_mode=non_streaming'),'Lab.19 debe instrumentar GPU, talker/code predictor y separar generación/decoder sin cambiar la voz a ciegas');
...
assert(runtime.includes('sampleCount=5')&&runtime.includes('nonStreamingMode:false')&&runtime.includes('chunkConvergencePct'),'Lab.19 debe comparar streaming incremental con N=5 y convergencia de chunks');
assert(worker.includes('ctypes.WinDLL("nvml.dll")')&&worker.includes('code_predictor_steps')&&worker.includes('torch.cuda.synchronize()'),'Lab.19 debe usar NVML en proceso y medir pasos del code predictor');
assert(baseOptimizerUi.includes('QWEN3-TTS LAB.19 · CANDIDATOS')&&baseOptimizerUi.includes('DIAGNÓSTICO GANADOR'),'Lab.19 debe mostrar el diagnóstico técnico comparativo');
...
assert(renderer.includes("window.__GEC_V2LAB_RENDERER_RESPONSIVE__='lab19'")&&release.includes('await labRuntime().importFineTunedZip'),'Hardening de responsividad lab.19 incompleto');
...
assert(baseOptimizerUi.includes('ttsLabel0321')&&baseOptimizerUi.includes('ttsLabStatus')&&baseOptimizerUi.includes("version:'2.0-lab.19'")&&baseOptimizerUi.includes('ttsLabStop')&&baseOptimizerUi.includes('ttsLabValidateSelected'),'Optimizador base V2 debe prevalidar Qwen, liberar TTS antes de Qwen local y usar la GPU CUDA real');
...
console.log('check-v2lab: OK · lab.19 · production profile · tuned fidelity · stable voice chunks · UI responsiva · CUDA isolation');
