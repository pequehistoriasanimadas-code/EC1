...
assert.strictEqual(pkg.version,'2.0.0-lab.10','La build debe identificarse como 2.0.0-lab.10');
...
assert(preload.includes('ttsLabSelectEngine')&&preload.includes('ttsLabImportReference')&&preload.includes('ttsLabPrepareReference')&&preload.includes('ttsLabUpdateReference')&&preload.includes('ttsLabImportFineTunedModel')&&preload.includes('ttsLabStop')&&preload.includes('ttsLabWarmupSelected')&&preload.includes('ttsLabValidateSelected'),'Bridge TTS Lab incompleto');
...
assert(runtime.includes("CACHE_REVISION='lab10-r1'")&&runtime.includes('coldStartRealtimeFactor')&&runtime.includes('stableRealtimeFactor')&&runtime.includes('async warmup'),'Runtime debe versionar caché, separar arranque en frío y mantener warm-up explícito');
...
console.log('check-v2lab: OK · lab.10 · Qwen fine-tuned exacto · reparación de assets · GPU SWAP · tuning por fingerprint · cleanup');
