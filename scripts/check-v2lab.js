...
assert(worker.includes('QWEN_PERF_REVISION = 2')&&worker.includes('_timed_qwen_wrapper')&&worker.includes('gpu_telemetry')&&worker.includes('non_streaming_mode=non_streaming'),'Lab.18 debe instrumentar GPU y separar generación autoregresiva/decoder sin cambiar la voz a ciegas');
assert(runtime.includes('stageTimings:r.stage_timings')&&runtime.includes('gpuTelemetry:r.gpu_telemetry')&&runtime.includes("bottleneck=gpuAvg>90?'gpu-bound'"),'Lab.18 debe propagar telemetría y clasificar el cuello de botella');
assert(baseOptimizerUi.includes('QWEN3-TTS LAB.19 · CANDIDATOS')&&baseOptimizerUi.includes('DIAGNÓSTICO GANADOR'),'Lab.18 debe mostrar el diagnóstico técnico comparativo');
