...
(async()=>{
  ...
  try{
    ...
    assert(!ui.includes('ttsLabValidateSelected')&&ui.includes('currentOptimizationKey')&&ui.includes('ttsModelFingerprint'),'UI lab.19 debe evitar la prevalidación duplicada e invalidar tuning al cambiar fine-tuned');
    assert(optimizerUi.includes("version:'2.0-lab.19'")&&optimizerUi.includes('Qwen local (texto)')&&optimizerUi.includes('GPU SWAP'),'Optimizador lab.19 debe distinguir texto/voz y mostrar GPU SWAP');
