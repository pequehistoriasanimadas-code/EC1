...
app.whenReady().then(()=>{let tmp='';try{
  const required=['src/bootstrap-v2lab.js','src/bootstrap-0332.js','src/services/release0332.js','src/renderer-0332.js','src/services/v2LabDataPath.js','src/services/releaseV2Lab.js','src/services/releaseV2Optimization.js','src/services/releaseV2Stabilization.js','src/renderer-stabilization-lab28.js','src/output-stabilization-lab28.js','src/services/releaseV2Lab29.js','src/renderer-lab29.js','src/services/ttsLabRuntime.js','src/renderer-v2lab.js','src/control-v2lab.css','src/tts_lab_worker.py','scripts/check-v2lab.js','scripts/check-v2lab-data-path.js','scripts/check-v2lab-stabilization-lab28.js','scripts/check-v2lab-lab29-youtube-monitor.js'];
  for(const rel of required)assert(fs.existsSync(path.join(appRoot,rel)),`Falta en app.asar: ${rel}`);
  const pkg=JSON.parse(fs.readFileSync(path.join(appRoot,'package.json'),'utf8'));
  assert(pkg.version==='2.0.0-lab.29','Versión empaquetada no es V2 TTS Lab 2.0.0-lab.29');
  ...
  assert(boot.includes('canonicalLabBase')&&boot.includes('GEC_V2_TTS_LAB_ROOT')&&boot.includes('recoverNestedLabData')&&boot.includes('releaseV2Stabilization')&&boot.includes('releaseV2Lab29'),'Bootstrap empaquetado debe mantener una sola ruta V2 Lab y cargar estabilización Lab.28 + Lab.29');
  ...
  const lab29Service=fs.readFileSync(path.join(appRoot,'src','services','releaseV2Lab29.js'),'utf8'),lab29Renderer=fs.readFileSync(path.join(appRoot,'src','renderer-lab29.js'),'utf8');
  assert(lab29Service.includes('preserveYoutubePromoState')&&lab29Service.includes('__youtubePromoConfigLab29'),'Guard de persistencia YouTube Lab.29 no está empaquetado');
  assert(lab29Renderer.includes('MONITOR_FPS=15')&&lab29Renderer.includes('MONITOR_BUSY_FPS=5')&&lab29Renderer.includes('monitorBusy'),'Monitor adaptativo 15/5 FPS Lab.29 no está empaquetado');
  ...
  assert(st.engines.some(x=>x.id==='chatterbox'&&!x.installed)&&st.engines.some(x=>x.id==='qwen3tts'&&!x.installed),'Motores experimentales deben iniciar bajo demanda');assert(Array.isArray(st.fineTunedModels)&&st.fineTunedModels.length===0,'Modelos fine-tuned deben iniciar vacíos y aislados');assert(typeof rt.importFineTunedZip==='function'&&typeof rt.updateReference==='function'&&typeof rt.prepareReference==='function'&&typeof rt.warmup==='function','Runtime empaquetado no expone flujo de voces V2 completo');
  const {normalizeProfileTts}=require(path.join(appRoot,'src','services','releaseV2Lab.js'));const profile=normalizeProfileTts({engine:'qwen3tts',style:'news',referenceVoiceId:'ref-test'});assert(profile.engine==='qwen3tts'&&profile.referenceVoiceId==='ref-test','Normalización de perfil TTS Lab falló');const cb=normalizeProfileTts({engine:'chatterbox',engineParams:{chatterbox:{}}});assert(cb.engineParams.chatterbox.variant==='latam','Chatterbox debe migrar por defecto al modelo LatAm');
  ...
  console.log('PACKAGED V2 TTS LAB lab.29 OK · YouTube persistente + monitor adaptativo · estabilización + LatAm · herencia 0.3.32 preservada');
  app.exit(0);
