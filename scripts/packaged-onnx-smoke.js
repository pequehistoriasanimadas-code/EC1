...
app.whenReady().then(async()=>{
  ...
  try{
    ...
    kokoro=new KokoroTTS({resourcesDir,dataDir:tempDir});assert(kokoro.ready(),'Kokoro empaquetado incompleto');const expectedParallelIntra=Math.min(4,kokoro.performanceThreadCap());let p=kokoro.profile();assert(p.executionMode==='parallel'&&p.onnxIntra===expectedParallelIntra&&p.inter===2,`el perfil paralelo 0.3.17 no llegó a Kokoro: esperado intra ${expectedParallelIntra}, recibido ${p.onnxIntra}, modo ${p.executionMode}`);assert(p.spinDurationUs===1000&&p.spinBackoffMax===8,'el perfil de spinning no llegó a Kokoro');
    ...
    fs.rmSync(tempDir,{recursive:true,force:true});console.log(`PACKAGED ONNX 0.3.17 OK · ORT_PARALLEL WAV · tiempos fonema/inferencia · AUTO CPU · spin 1 ms/backoff 8 · cap=${expectedParallelIntra} · voz=${voice}`);app.exit(0);
