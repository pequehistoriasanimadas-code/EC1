...
app.whenReady().then(async()=>{let tmp='';try{
  ...
  assert(renderer.includes("window.__GEC_V2LAB_RENDERER_RESPONSIVE__='lab16'"),'Marcador de renderer lab.16 no empaquetado');
  assert(optimizer.includes("version:'2.0-lab.16'")&&optimizer.includes('GEC valida ambos motores'),'Optimizador lab.16 no empaquetado');
  ...
  tmp=fs.mkdtempSync(path.join(os.tmpdir(),'gec-packaged-lab16-'));
  ...
  console.log('PACKAGED V2 RESPONSIVE lab.16 OK · no spawnSync · q fixed · session CUDA cache · event loop alive');
  app.exit(0);
