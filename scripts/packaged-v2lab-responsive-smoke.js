...
app.whenReady().then(async()=>{let tmp='';try{
  ...
  assert(renderer.includes("window.__GEC_V2LAB_RENDERER_RESPONSIVE__='lab20'"),'Marcador de renderer lab.23 no empaquetado');
  assert(optimizer.includes("version:'2.0-lab.23'")&&optimizer.includes('GEC valida ambos motores'),'Optimizador lab.23 no empaquetado');
