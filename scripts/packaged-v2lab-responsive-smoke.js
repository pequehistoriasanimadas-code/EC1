...
app.whenReady().then(async()=>{let tmp='';try{
  ...
  assert(renderer.includes("window.__GEC_V2LAB_RENDERER_RESPONSIVE__='lab25'"),'Marcador de renderer lab.23 no empaquetado');
  assert(optimizer.includes("version:'2.0-lab.25'")&&optimizer.includes('GEC valida ambos motores'),'Optimizador lab.23 no empaquetado');
  ...
  console.log(`PACKAGED V2 RESPONSIVE ${pkg.version} OK · no spawnSync · q fixed · session CUDA cache · event loop alive`);
  app.exit(0);
