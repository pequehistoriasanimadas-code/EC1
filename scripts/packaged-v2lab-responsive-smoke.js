...
app.whenReady().then(async()=>{let tmp='';try{
  ...
  assert.strictEqual(pkg.version,'2.0.0-lab.13');
  ...
  assert(source.includes("const existing=this.workers.get(id);if(existing?.child&&existing.ready)return existing;await this.ensureCudaRuntimeHealthy()"),'Worker reuse-first no empaquetado');
  ...
  assert(renderer.includes("window.__GEC_V2LAB_RENDERER_RESPONSIVE__='lab13'"),'Marcador de renderer lab.13 no empaquetado');
  assert(optimizer.includes("version:'2.0-lab.13'")&&optimizer.includes('GEC valida ambos motores'),'Optimizador lab.13 no empaquetado');
  ...
  tmp=fs.mkdtempSync(path.join(os.tmpdir(),'gec-packaged-lab13-'));
  ...
  console.log('PACKAGED V2 RESPONSIVE lab.13 OK · no spawnSync · q fixed · session CUDA cache · event loop alive');
  app.exit(0);
