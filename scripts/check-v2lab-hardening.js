...
(async()=>{
  ...
  assert(runtimeSource.includes('cuda-transaction.json')&&runtimeSource.includes('activation-pending')&&runtimeSource.includes('recoverLegacyCudaOrphans'),'Falta journal/recovery CUDA lab.15');
  ...
  assert(renderer.includes('v2InstallProgress')&&renderer.includes('Reintentar activación')&&renderer.includes('Instalación / preparación incompleta'),'UX de progreso/error persistente lab.15 incompleta');
  ...
  assert(release.includes("browser-window-created")&&release.includes("control\\.html")&&release.includes("setTimeout(recover,8000)")&&!release.includes("setTimeout(recover,50)"),'La recuperación TTS debe esperar a la UI principal en lab.15');

  const base=fs.mkdtempSync(path.join(os.tmpdir(),'gec-lab15-hardening-'));
  try{
    ...
    // Upgrade from lab.13: orphan backup is recovered even without lab.15 journal.
    {
