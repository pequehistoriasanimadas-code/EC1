...
for(const file of [
  ...
  'src/renderer-emission-output-ux-lab29.js',
  'src/control-emission-output-ux-lab29.css',
...
const repairUi=read('src/renderer-emission-output-ux-lab29.js');
const repairCss=read('src/control-emission-output-ux-lab29.css');
const repairRelease=read('src/services/releaseV2UxRepairLab29.js');

// Salida compacta: el único Abrir Output visible es el global; la configuración técnica queda plegada.
assert(repairUi.includes('compactOutputCards'),'Salida debe compactar los paneles existentes sin reemplazar su lógica');
assert(repairUi.includes("q('#ecOutputMasterOpen')")&&repairUi.includes('duplicate?.remove()'),'La reparación debe retirar el botón duplicado de Output maestro');
for(const token of ['ec-output-permissions-compact','ec-output-lan-advanced','ec-output-ndi-advanced'])assert(repairUi.includes(token),`Falta compactación de Salida: ${token}`);
for(const token of ['ec-output-permissions-compact','ec-output-lan-advanced','ec-output-ndi-advanced'])assert(repairCss.includes(token),`Falta CSS compacto de Salida: ${token}`);
assert(repairUi.includes("permissionsHost?.classList.add('ec-output-wide')"),'Permisos de red debe ocupar una fila compacta completa');
assert(repairUi.includes("ndiHost?.classList.remove('ec-output-wide')"),'LAN y NDI deben aprovechar las dos columnas cuando hay espacio');
...
for(const token of ['ecV2PreviewToolbar','ecV2Format16','ecV2Format916','ec-v2-format-buttons'])assert(repairUi.includes(token),`Falta selector segmentado de Diseño: ${token}`);
assert(repairUi.includes('toolbar.appendChild(modeBar)'),'Nota/Promo debe moverse a la barra de la preview');
assert(repairUi.includes("format.dispatchEvent(new Event('change'"),'Los botones 16:9 / 9:16 deben reutilizar el flujo existente de cambio de formato');
assert(repairUi.includes('ec-v2-format-source-hidden'),'El selector legacy debe quedar en DOM pero oculto para preservar compatibilidad');
assert(repairUi.includes('ecV2VerticalQuickControls'),'Safe zone, guías y fondo 9:16 deben vivir debajo de la barra de preview');
assert(repairUi.includes('ec-v2-preview-legacy-head'),'El título redundante de Vista previa debe ocultarse sin borrar IDs usados por el renderer');
for(const token of ['ec-v2-preview-toolbar','ec-v2-format-buttons','ec-v2-vertical-quick-controls','ec-v2-format-source-hidden'])assert(repairCss.includes(token),`Falta CSS de barra compacta de Diseño: ${token}`);
assert(repairRelease.includes('control-emission-output-ux-lab29.css')&&repairRelease.includes('renderer-emission-output-ux-lab29.js'),'La capa final de UX debe inyectarse en control.html');
...
assert((pkg.build.files||[]).includes('src/**/*'),'La capa de reparación debe quedar incluida por src/**/* en el paquete');
