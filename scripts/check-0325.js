...
function makeEngine(){const e=Object.create(AutomationEngine.prototype);Object.assign(e,{queue:[],selectionRecent:[],urlFailures:new Map(),feedFailures:new Map(),liveBaseCooldown:new Map(),omittedSources:new Map(),queuedUrls:new Set(),exclusiveReserve:[],exclusiveReserveKeys:new Set(),newsSinceExclusive:0,exclusiveHasEmitted:false,newsStatuses:new Map(),emissionHistory:[],currentKind:'none',currentCanned:null,history:{has:()=>false},getSettings:()=>({automation:{exclusiveEveryNews:4,maxAgeHours:6,avoidRepeats:true},rssFeeds:[],canned:{enabled:false}}),isFeedActive:()=>true,addEmissionHistory:()=>{}});return e;}
...
{
 const e=makeEngine();const s={...e.getSettings(),rssFeeds:[{id:'x',enabled:true,url:'https://x.test',accessMode:'exclusive'},{id:'p',enabled:true,url:'https://p.test',accessMode:'auto'}]};e.getSettings=()=>s;const picked=e.candidateFrom([{link:'https://p.test/1',feedId:'p',pubDate:now},{link:'https://x.test/1',feedId:'x',feedAccessMode:'exclusive',pubDate:ago(25)}],s,{publicOnly:true});assert.equal(picked.feedId,'p');assert.equal(picked.__ecSelectionReason,'actualidad + variedad de fuente');
}
{
 const e=makeEngine(),s=e.getSettings(),pub={status:'LISTA',story:{title:'P'},result:{isExclusive:false}},ex={status:'LISTA',story:{title:'E'},result:{isExclusive:true}};e.newsSinceExclusive=1;assert.equal(e.chooseReadyItem([ex,pub],s),pub);e.newsSinceExclusive=3;assert.equal(e.chooseReadyItem([pub,ex],s),ex);e.newsSinceExclusive=1;assert.equal(e.chooseReadyItem([ex],s),null);
}
{
