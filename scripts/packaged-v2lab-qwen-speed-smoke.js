...
app.whenReady().then(async()=>{try{
  ...
  assert.strictEqual(pkg.version,'2.0.0-lab.21');
  ...
  for(const token of ['QWEN_PERF_REVISION = 4','predictorHiddenStates','subtalkerTopK','batchSize','generate_qwen_finetuned_batch','kwargs["output_hidden_states"] = False','code_predictor_ms_per_step'])assert(worker.includes(token),`Qwen lab.21 worker missing ${token}`);
  for(const token of ['QWEN3-TTS LAB.21 · SAMPLING (N=5)','QWEN3-TTS LAB.21 · BATCHING (N=2 + perfil)','DIAGNÓSTICO GANADOR LAB.21','batchEffectiveRealtimeFactor',"version:'2.0-lab.21'"])assert(ui.includes(token),`Unified optimizer lab.21 UI missing ${token}`);
  assert(v2.includes("window.__GEC_V2LAB_RENDERER_RESPONSIVE__='lab20'"),'Renderer lab.21 marker missing');
  console.log('PACKAGED V2 QWEN SPEED lab.21 OK · sampling · hidden OFF · predictor top-k · B1/B2/B4/B8 · VRAM guard');
  app.exit(0);
