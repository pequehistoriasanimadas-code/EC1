...
app.whenReady().then(async()=>{try{
  ...
  for(const token of ['QWEN3-TTS LAB.23 · SAMPLING (N=5)','QWEN3-TTS LAB.23 · BATCHING (N=2 + perfil)','DIAGNÓSTICO GANADOR LAB.23','batchEffectiveRealtimeFactor',"version:'2.0-lab.23'"])assert(ui.includes(token),`Unified optimizer lab.23 UI missing ${token}`);
  assert(v2.includes("window.__GEC_V2LAB_RENDERER_RESPONSIVE__='lab20'"),'Renderer lab.23 marker missing');
  console.log('PACKAGED V2 QWEN SPEED lab.23 OK · sampling · hidden OFF · predictor top-k · B1/B2/B4/B8 · VRAM guard');
  app.exit(0);
