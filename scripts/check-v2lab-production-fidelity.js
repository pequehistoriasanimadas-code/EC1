...
(async()=>{
  ...
  assert(preload.includes('optimizationV2Status')&&preload.includes('optimizationV2Commit'),'Bridge lab.18 incompleto');
  assert(bootstrap.includes('releaseV2ProductionFidelity'),'Capa lab.18 no instalada');
  ...
  try{
    ...
    const profile={
      schemaVersion:1,version:'2.0-lab.18',id:'test-profile',at:new Date().toISOString(),
