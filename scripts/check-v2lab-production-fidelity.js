...
(async()=>{
  ...
  assert(preload.includes('optimizationV2Status')&&preload.includes('optimizationV2Commit'),'Bridge lab.15 incompleto');
  assert(bootstrap.includes('releaseV2ProductionFidelity'),'Capa lab.15 no instalada');
  ...
  const tmp=fs.mkdtempSync(path.join(os.tmpdir(),'gec-lab15-'));
  try{
    ...
    const profile={
      schemaVersion:1,version:'2.0-lab.15',id:'test-profile',at:new Date().toISOString(),
