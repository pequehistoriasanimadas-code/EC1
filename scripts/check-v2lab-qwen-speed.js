...
(async()=>{
  ...
  for(const token of ['QWEN_PERF_REVISION = 4','predictorHiddenStates','subtalkerTopK','batchSize','generate_qwen_finetuned_batch','kwargs["output_hidden_states"] = False','code_predictor_ms_per_step'])assert(worker.includes(token),`Worker lab.20 missing ${token}`);
  for(const token of ['sampleCount=5','batchSampleCount=2','subtalkerTopK:20','subtalkerTopK:10','batchSizes=String(params.voiceMode||\'reference\')===\'finetuned\'?[1,2,4,8]','batchVramHeadroomMb','stableRuns=id===\'qwen3tts\'?5:3'])assert(runtime.includes(token),`Runtime lab.20 missing ${token}`);
  assert(!runtime.includes("add('fp16-sdpa'"),'Lab.20 no debe reintroducir FP16');
  console.log('check-v2lab-qwen-speed: OK · lab.20 · sampling audit · hidden-state optimization · batching · VRAM guard');
