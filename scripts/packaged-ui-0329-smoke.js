...
async function makeWindow(){const win=new BrowserWindow({show:false,width:1280,height:820,webPreferences:{preload,contextIsolation:true,nodeIntegration:false,sandbox:false}});win.webContents.on('console-message',(_,level,message)=>{if(level>=3)console.error('renderer:',message);});await win.loadFile(control,{query:{selftest:'1'}});return win;}
...
app.whenReady().then(async()=>{registerIpc();let win=null;try{
  ...
  await win.webContents.executeJavaScript(`document.querySelector('#ec29CreateFirst')?.click();`,true);await until(win,"!!document.querySelector('#ec29ProfileInput')");
  const input=await win.webContents.executeJavaScript(`(()=>{const x=document.querySelector('#ec29ProfileInput');x.value='Prueba';x.dispatchEvent(new Event('input',{bubbles:true}));return {disabled:x.disabled,value:x.value,preview:document.querySelector('#ec29ProfilePreview')?.textContent||''};})()`,true);assert(!input.disabled&&input.value==='Prueba','El campo Nombre del perfil no acepta edición');assert(/PRUEBA/i.test(input.preview),'La vista previa del nombre no responde al input');
  ...
  win.destroy();win=null;console.log('PACKAGED REAL UI 0.3.29 OK · startup visible · onboarding · input editable · selector ACTIVO');app.exit(0);
