...
(async()=>{
  ...
  assert(runtimeSource.includes("const existing=this.workers.get(id);if(existing?.child&&existing.ready)return existing;await this.ensureCudaRuntimeHealthy()"),'ensureWorker debe reutilizar primero un worker vivo antes de validar CUDA');
  ...
  assert(renderer.includes("window.__GEC_V2LAB_RENDERER_RESPONSIVE__='lab13'"),'Renderer debe identificar el hardening de responsividad lab.13');
  ...
  const temp=fs.mkdtempSync(path.join(os.tmpdir(),'gec-lab13-responsive-'));
  try{
