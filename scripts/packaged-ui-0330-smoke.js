...
function registerIpc(){
  ...
  bind('automation:status',()=>({processing:{running:false,paused:false,message:'Smoke 0.3.30'},emission:{running:false,paused:false,currentTitle:'',currentKind:'none'},counts:{processing:0,ready:1,pending:0,onAir:0,emitted:0,error:0},buffer:{target:15,autonomyMin:.8,health:'critical'},queue:[{id:'r1',title:'Noticia lista',status:'LISTA',sourceType:'rss',queueGroup:'effective',displayPosition:1},{id:'p1',title:'Noticia preparando',status:'PROCESANDO',sourceType:'rss',stage:'ai',queueGroup:'preparing',displayPosition:0}],session:{newsEmitted:0,cannedEmitted:0,adsEmitted:0},canned:{enabled:false},ads:{enabled:false},documents:{pending:0,processing:false}}));
...
app.whenReady().then(async()=>{let win=null;try{
  ...
  profileMode='profiles';win=await makeWindow();await until(win,"!!document.querySelector('#ec29ProfileButton')");await until(win,"window.__ec0330Installed===true");
  ...
  await js(win,"document.querySelector('[data-tab=auto]')?.click(); true;");await wait(500);const queue=await js(win,"({ready:[...document.querySelectorAll('#queue .queue-index')].map(x=>x.textContent.trim()),divider:!!document.querySelector('.ec0330-preparing-divider'),overflow:document.documentElement.scrollWidth>document.documentElement.clientWidth})");assert(queue.ready.includes('1.'),'La cola efectiva no usa numeración 1..N');assert(queue.divider,'No aparece separación En preparación');
  win.setSize(900,760);await wait(250);const horizontal=await js(win,"document.documentElement.scrollWidth>document.documentElement.clientWidth");assert(!horizontal,'La UI 0.3.30 genera scroll horizontal a ancho intermedio');
  win.destroy();console.log('PACKAGED REAL UI 0.3.30 OK · onboarding cancel · tuning global · cola efectiva · preparación separada · responsive');app.exit(0);
