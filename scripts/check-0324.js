...
ok('0.3.24 UI and Output assets are wired, not merely packaged',()=>{
  const renderer=fs.readFileSync(path.join(__dirname,'../src/renderer-0324.js'),'utf8'),preload=fs.readFileSync(path.join(__dirname,'../src/preload.js'),'utf8'),output=fs.readFileSync(path.join(__dirname,'../src/output-0324.js'),'utf8'),controlHtml=fs.readFileSync(path.join(__dirname,'../src/control.html'),'utf8'),outputHtml=fs.readFileSync(path.join(__dirname,'../src/output.html'),'utf8'),outputCss=fs.readFileSync(path.join(__dirname,'../src/output-0324.css'),'utf8'),automation=fs.readFileSync(path.join(__dirname,'../src/services/automation0324.js'),'utf8');assert(renderer.includes('customVoiceManager'));assert(renderer.includes('fmtDuration(x.durationSec)'));assert(renderer.includes('transitionType'));assert(renderer.includes('pronDownloadBusy0324'));assert(renderer.includes('localDownloadBusy0324'));assert(preload.includes('voices:import'));assert(preload.includes('fonts:refresh'));assert(output.includes('exclusiveBadge'));assert(!controlHtml.includes('control-0324.css'));assert(!controlHtml.includes('renderer-0324.js'));assert(preload.includes("href:'control-0324.css'"));assert(preload.includes("src:'renderer-0324.js'"));assert(outputHtml.includes('output-0324.css'));assert(outputHtml.includes('output-0324.js'));assert(outputCss.includes('font-family:var(--date-font-family)'));assert(automation.includes('article.isLiveBlog&&article.liveEvent'));
});
