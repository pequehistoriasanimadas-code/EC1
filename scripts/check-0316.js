...
const pkg=JSON.parse(read('package.json'));ok(['0.3.16','0.3.17','0.3.18','0.3.19','0.3.20'].includes(pkg.version),'package version debe ser 0.3.16 o posterior compatible');
...
try{
  ...
  const pron=new PronunciationNormalizer({resourcesDir:temp,dataDir:temp,getSettings:()=>({ai:{primary:'local'},tts:{pronunciationSmart:true,pronunciationClaudeVerify:false,pronunciationMaxSeconds:15}})});
  ...
  const settings={ai:{primary:'local'},tts:{},automation:{bufferReady:15,queueMax:30,targetAutonomyMin:15,generatedEveryRss:5,recoveryAutonomyMin:8,criticalAutonomyMin:3},canned:{enabled:true,interval:10,folder:'C',adsFolder:'A',insertAdAfterContent:true,emergency:true,adaptiveDuration:true},visual:{pauseSeconds:0}};
  const makeEngine=()=>new AutomationEngine({rss:{},fetchArticle:null,providers:null,kokoro:{status:()=>({recentRealtimeFactor:.8,recentSamples:3})},pronunciation:null,canned:{peek:()=>({name:'CONTENIDO.mp4'}),peekForDuration:()=>({name:'CONTENIDO.mp4'}),requestDuration:()=>{}},ads:{peek:()=>({name:'ANUNCIO.mp4'})},history:{file:path.join(temp,'history.json'),add:()=>{}},getSettings:()=>settings,getFallbackUrl:()=>'',sendAutomaticOutput:()=>true,isOutputReady:()=>true,controlOutput:()=>{}});
