...
try{
  // El guard del backend debe impedir que un save con datos anteriores pise la
  // recomendación de hilos ganadora del benchmark.
  ...
  // Importar un cambio de Bloc de notas debe transformarlo en regla MANUAL y
  // ningún aprendizaje automático posterior puede sobrescribirla.
  ...
  const beforeUses=mx.uses;const normalizedPromise=pron.normalize('MX ganó.',{smart:true});
  Promise.resolve(normalizedPromise).then(()=>{});
  // normalize es async; el resto del check continúa dentro de una rutina async.
  ...
  (async()=>{
    ...
    // Documento generado: ready se persiste; tras reinicio vuelve a pendiente si
    // nunca fue emitido; al emitirse queda registrado de forma permanente.
    const doc={fingerprint:'doc-0316',path:path.join(temp,'nota.txt'),title:'Nota 0316'};fs.writeFileSync(doc.path,'texto');
    const e1=makeEngine();e1.__ecSetDocumentProcessed(doc,true);let rec=new SettingsStore(temp).load().documents.processed[doc.fingerprint];ok(rec?.status==='ready'&&rec.generatedAt&&!rec.emittedAt,'documento listo no persiste generatedAt/status ready');
    const e2=makeEngine();e2.snapshot();rec=new SettingsStore(temp).load().documents.processed[doc.fingerprint];ok(!rec,'una nota lista pero no emitida queda bloqueada después de reiniciar');
    e2.__ecSetDocumentProcessed(doc,true);e2.currentItem={sourceType:'generated',document:doc,metrics:{},audio:{durationSec:30}};e2.addEmissionHistory('generated',doc.title,'EMITIDA',{durationSec:30});rec=new SettingsStore(temp).load().documents.processed[doc.fingerprint];ok(rec?.status==='emitted'&&rec.emittedAt&&rec.generatedAt,'documento emitido no conserva estado/timestamps');e2.snapshot();const listed=augmentDocumentList({files:[{fingerprint:doc.fingerprint,processed:true}]});ok(listed.files[0].documentStatus==='emitted'&&listed.files[0].documentStatusLabel==='Emitida','documents:list no traduce el estado persistido a Emitida');

    // Mientras el contenido está al aire, el anuncio reservado sigue visible.
