...
app.whenReady().then(async()=>{let tmp='';try{
  ...
  assert.strictEqual(pkg.version,'2.0.0-lab.15','Versión empaquetada no es lab.15');
  ...
  assert(ui.includes('currentOptimizationKey')&&!ui.includes('ttsLabValidateSelected')&&ui.includes('v2InstallProgress'),'UI fine-tuned lab.15 no está empaquetada');

  fs.rmSync(tmp,{recursive:true,force:true});tmp='';
  console.log('PACKAGED V2 QWEN FINE-TUNED lab.15 OK · Aurelio exacto · overlay inmutable · fingerprint · GPU SWAP');
  app.exit(0);
