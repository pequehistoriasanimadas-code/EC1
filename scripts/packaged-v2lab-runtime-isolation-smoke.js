...
app.whenReady().then(async()=>{let tmp='';try{
  ...
  assert.strictEqual(pkg.version,'2.0.0-lab.12');
  ...
  tmp=fs.mkdtempSync(path.join(os.tmpdir(),'GEC ÁREA DISEÑO packaged lab12 '));
  const rt=new TTSLabRuntime({resourcesDir,dataDir:tmp});
  rt.freeBytes=()=>20*1024*1024*1024;rt.nvidiaPresent=()=>true;
  rt.validateCudaSite=site=>({ok:true,torch:'2.6.0',torchaudio:'2.6.0',cuda:true,torch_cuda:'12.4',site});
  rt.validateInstalledEnginesAgainstCuda=()=>[];
  let installs=0;
  ...
  assert(source.includes('validateInstalledEnginesAgainstCuda')&&source.includes('crossFailures.length'),'Prueba cruzada Chatterbox/Qwen no empaquetada');
  ...
  console.log('PACKAGED V2 RUNTIME ISOLATION lab.12 OK · CUDA v2 · mutex · transaction · torch_version corruption · Unicode path');
  app.exit(0);
