...
const safety=read('src/services/version0318SafetyPolicy.js');ok(/gpuBaseline/.test(safety)&&/gpuDelta/.test(safety)&&/useCpuAcceleration/.test(safety)&&/tts:gpuUseCpu/.test(safety),'falta medición base GPU o restauración manual de CPU');
const preload=read('src/preload.js');ok(/gpuTtsStatus/.test(preload)&&/installGpuTts/.test(preload)&&/benchmarkGpuTts/.test(preload)&&/useCpuTts/.test(preload)&&/tts:gpuEvent/.test(preload),'preload no expone controles GPU/CPU');
const ui=read('src/renderer-0318.js');ok(/Aceleración NVIDIA/.test(ui)&&/Comparar CPU vs GPU/.test(ui)&&/GPU antes de Kokoro/.test(ui)&&/Usar CPU/.test(ui)&&/fallback/.test(ui),'UI GPU no presenta baseline/comparación/fallback/restauración CPU');
const loader=read('src/services/version0318RendererLoader.js');ok(/version0318SafetyPolicy/.test(loader)&&/renderer-0318\.js/.test(loader),'capa de seguridad/renderer 0.3.18 no se carga');
