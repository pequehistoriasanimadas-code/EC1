...
const policy=read('src/services/version0318Policy.js');for(const token of ['onnxruntime-gpu[cuda,cudnn]==1.26.0','CUDA_MIN_DRIVER=525','gpu-runtime','benchmarkGpu','cuda-fallback','gpu_mem_limit'])ok(policy.includes(token),`falta ${token} en política GPU 0.3.18`);
...
try{
  ...
  fs.rmSync(gpuDir,{recursive:true,force:true});p=kokoro.profile();ok(p.provider==='cpu','si runtime GPU falta, Kokoro no vuelve automáticamente a CPU');
